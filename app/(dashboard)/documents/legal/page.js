    ...
    <div className="p-4 sm:p-6 space-y-5">
        ...
        <div className="relative flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h1 className="text-xl font-bold" style={{ color: '#ffffff' }}>المستندات القانونية</h1>
              <p className="text-white/50 text-sm mt-0.5">{list.length} مستند</p>
            </div>
          </div>
          <Link href="/documents/legal/create"
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 bg-white rounded-2xl border border-[#E2E6F0] shadow-[0_2px_8px_rgba(8,26,58,0.05)]">
          ...
          <p className="text-[#0A1628] font-semibold">لا توجد مستندات قانونية</p>
        </div>
      ) : (
            ...
            <table className="w-full text-sm">
              <thead style={{ background: '#F0F4FA' }} className="border-b border-[#E2E6F0]">
                <tr>
                  ...
                  <th className={thCls}>نوع المستند</th>
                  <th className={thCls}>رقم المستند</th>
                  <th className={thCls}>رقم الوكالة</th>
              ...
              <tbody className="divide-y divide-[#F0F2F7]">
                {list.map((d) => (
                  <tr key={d.id} className="hover:bg-[#F8F9FC] transition-colors">
                    <td className="px-4 py-3.5 font-semibold text-[#0A1628]">{d.customer?.name || `#${d.customer_id}`}</td>
                    <td className="px-4 py-3.5 text-[#4A5568]">{DOC_TYPES[d.document_type] || d.document_type}</td>
                    <td className="px-4 py-3.5 font-mono text-[#4A5568]">{d.document_number}</td>
                    <td className="px-4 py-3.5 text-[#8896A7]">{d.agency_number || '—'}</td>
                    ...
                    <td className="px-4 py-3.5 text-[#8896A7] max-w-xs truncate">{d.description || '—'}</td>
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-1.5 justify-end">
                        <Link href={`/documents/legal/${d.id}`}
                        ...
                        <Link href={`/documents/legal/${d.id}/edit`}
                          className="w-7 h-7 flex items-center justify-center rounded-lg bg-[#EBF0FA] text-[#081A3A] hover:bg-[#081A3A]/15 transition-colors">
                          <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
