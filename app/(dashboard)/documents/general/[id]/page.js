...
export default function GeneralDocDetailPage() {
  ...
  const deleteFileMutation = useMutation({
    mutationFn: (index) => generalDocsApi.deleteFile(tenantApi, id, index),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['general-document', id] }),
  });
  ...
  return (
    <div className="p-6 max-w-2xl space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href={`/documents/general/${id}/edit`}><Button>تعديل</Button></Link>
      </div>

      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <h2 className="font-semibold text-gray-800 mb-4">بيانات المستند</h2>
        <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field label="نوع الملف" value={doc.file_type} />
              ...
              <li key={index} className="flex items-center gap-3 py-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="text-xs text-red-500 hover:text-red-700">حذف</button>
                </div>
              </li>
            ))}
          </ul>
