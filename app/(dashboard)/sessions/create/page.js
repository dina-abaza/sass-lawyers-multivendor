...
export default function CreateSessionPage() {
  ...
  const [form, setForm] = useState({
    case_id: '', user_id: '', case_status_id: '',
    session_number: '', court_side: '', agency: '',
    day: '', date: '', date_hijri: '',
    session_time: '', reminder_date: '', notes: '',
  });
  ...
  return (
    <div className="p-6 max-w-2xl">
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Select label="حالة القضية" name="case_status_id" value={form.case_status_id} onChange={handleChange} options={toOptions(statuses)} required />
          <Input label="رقم الجلسة" name="session_number" value={form.session_number} onChange={handleChange} />
        </div>

        {/* الجهة القضائية + الجهة */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
