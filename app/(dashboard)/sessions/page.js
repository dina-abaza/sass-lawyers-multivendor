    ...
    <div className="p-4 sm:p-6 space-y-5">
        ...
        <div className="relative flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h1 className="text-xl font-bold" style={{ color: '#ffffff' }}>جلسات المحكمة</h1>
              <p className="text-white/50 text-sm mt-0.5">{list.length} جلسة</p>
            </div>
          </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 bg-white rounded-2xl border border-[#E2E6F0] shadow-[0_2px_8px_rgba(8,26,58,0.05)]">
          ...
          <p className="text-[#0A1628] font-semibold">لا توجد جلسات مسجلة</p>
        </div>
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead style={{ background: '#F0F4FA' }} className="border-b border-[#E2E6F0]">
                <tr>
                  <th className={thCls}>رقم الجلسة</th>
                  <th className={thCls}>القضية</th>
                  <th className={thCls}>الجهة القضائية</th>
              ...
              <tbody className="divide-y divide-[#F0F2F7]">
                {list.map((s) => (
                  <tr key={s.id} className="hover:bg-[#F8F9FC] transition-colors">
                    <td className="px-4 py-3.5 font-bold text-[#081A3A]">{s.session_number || `#${s.id}`}</td>
                    <td className="px-4 py-3.5 text-[#4A5568]">{s.legal_case?.case_number || s.case_id}</td>
                    <td className="px-4 py-3.5 text-[#4A5568]">{s.court_side || '—'}</td>
                    <td className="px-4 py-3.5 text-[#4A5568]">{s.agency || '—'}</td>
                    <td className="px-4 py-3.5 text-[#8896A7]">{s.date || '—'}</td>
                    <td className="px-4 py-3.5 text-[#8896A7]">{s.session_time || '—'}</td>
                    <td className="px-4 py-3.5">
                      {s.status?.name ? (
                        <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-[#EBF0FA] text-[#081A3A] ring-1 ring-inset ring-[#081A3A]/15">
                          {s.status.name}
                        </span>
                      ) : '—'}
                    </td>
