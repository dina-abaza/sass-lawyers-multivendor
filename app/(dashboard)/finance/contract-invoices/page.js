...
export default function ContractInvoicesPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => invoicesApi.getContractInvoices(tenantApi).then((r) => r.data),
    enabled: !!tenantApi,
  });
        ...
        <div className="relative flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h1 className="text-xl font-bold" style={{ color: '#ffffff' }}>فواتير العقود</h1>
              <p className="text-white/50 text-sm mt-0.5">دفعات واستحقاقات عقود الاتفاقية مع العملاء</p>
            </div>
          </div>
          <Link href="/finance/contract-invoices/create"
              ...
              <tbody className="divide-y divide-[#F0F2F7]">
                {list.map((inv) => (
                  <tr key={inv.id} className="hover:bg-[#F8F9FC] transition-colors">
                    ...
                    <td className="px-4 py-3.5 font-semibold text-[#0A1628]">{inv.contract?.name || inv.contract_id}</td>
