    ...
    <div className="p-4 sm:p-6 space-y-5">
        ...
        <div className="relative flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h1 className="text-xl font-bold" style={{ color: '#ffffff' }}>سندات الصرف</h1>
            </div>
          </div>
          <Link href="/finance/payment-vouchers/create"
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 bg-white rounded-2xl border border-[#E2E6F0] shadow-[0_2px_8px_rgba(8,26,58,0.05)]">
          <p className="text-[#8896A7] text-sm">لا توجد سندات صرف</p>
        </div>
      ) : (
              ...
              <tbody className="divide-y divide-[#F0F2F7]">
                {list.map((v) => (
                  <tr key={v.id} className="hover:bg-[#F8F9FC] transition-colors">
                    <td className="px-4 py-3.5 font-semibold text-[#0A1628]">{v.customer?.name || v.customer_id}</td>
                    <td className="px-4 py-3.5 font-bold text-red-600">{v.amount?.toLocaleString()} ر.س</td>
                    <td className="px-4 py-3.5 text-[#4A5568]">{v.for_reason}</td>
                    <td className="px-4 py-3.5 text-[#4A5568]">{v.voucher_date}</td>
