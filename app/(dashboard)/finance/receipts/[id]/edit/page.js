...
export default function EditReceiptPage() {
  ...
  useEffect(() => {
    const list = Array.isArray(allReceipts) ? allReceipts : allReceipts?.data ?? [];
    const receipt = list.find((r) => String(r.id) === String(id));
    if (receipt) {
      setForm({
        customer_id: receipt.customer_id ?? '',
        receipt_date: receipt.receipt_date ?? '',
        receipt_date_hijri: receipt.receipt_date_hijri ?? '',
        amount: receipt.amount ?? '',
        amount_text: receipt.amount_text ?? '',
        for_reason: receipt.for_reason ?? '',
        notes: receipt.notes ?? '',
  ...
  const mutation = useMutation({
    mutationFn: (data) => receiptsApi.update(tenantApi, id, data),
        ...
        <div className="relative flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h1 className="text-xl font-bold" style={{ color: '#ffffff' }}>تعديل سند القبض</h1>
            </div>
          </div>
