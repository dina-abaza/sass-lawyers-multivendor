...
export default function EditVacationPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => vacationsApi.getAll(tenantApi).then((r) => {
      const list = Array.isArray(r.data) ? r.data : r.data?.data ?? [];
      return list.find((v) => String(v.id) === String(id));
    }),
  ...
  const mutation = useMutation({
    mutationFn: (payload) => vacationsApi.update(tenantApi, id, payload),
    onSuccess: () => {
    ...
    <div className="p-4 sm:p-6 space-y-5 max-w-md mx-auto">
        ...
        <div className="relative flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h1 className="text-xl font-bold" style={{ color: '#ffffff' }}>تعديل الإجازة</h1>
            </div>
          </div>
      ...
      <div className="bg-white rounded-2xl border border-[#E2E6F0] p-6 shadow-[0_2px_8px_rgba(8,26,58,0.05)]">
        ...
        <form onSubmit={(e) => { e.preventDefault(); setError(null); mutation.mutate(form); }} className="space-y-5">
          ...
          <Select label="الموظف" name="employee_id" value={String(form.employee_id)}
            onChange={handleChange} options={toOptions(employees)} required />
