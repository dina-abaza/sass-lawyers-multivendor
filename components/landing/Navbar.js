    ...
    <header className={`sticky top-0 z-50 transition-shadow duration-300 ${scrolled ? 'bg-white/95 backdrop-blur-md shadow-sm shadow-navy-900/5' : 'bg-white'} border-b border-[#e4e9f2]`}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-4">
        ...
        <a href="#home" onClick={e => scrollTo(e, 'home')} className="flex items-center gap-2.5 shrink-0">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: "linear-gradient(135deg, #081A3A, #0D2452)", boxShadow: "0 4px 8px rgba(8,26,58,0.25)" }}>
        ...
        <div className="hidden sm:flex items-center gap-2 shrink-0">
          ...
          <Link href="/register" className="px-5 py-2 rounded-full text-sm font-semibold transition-all" style={{ background: "#081A3A", color: "#ffffff" }}>
            ابدأ مجاناً
          </Link>
        </div>
      ...
      {open && (
        <div className="md:hidden border-t border-[#e4e9f2] bg-white px-4 py-4 space-y-1">
          ...
          <div className="flex items-center gap-2 pt-3 border-t border-[#e4e9f2]">
            ...
            <Link href="/register" className="flex-1 text-center px-4 py-2.5 rounded-full text-sm font-semibold" style={{ background: "#081A3A", color: "#ffffff" }}>
              ابدأ مجاناً
            </Link>
          </div>
        </div>
