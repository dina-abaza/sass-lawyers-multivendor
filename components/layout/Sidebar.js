...
export default function Sidebar({ mobileOpen, onMobileClose }) {
  ...
  const sidebarContent = (
    <aside className="w-64 h-full bg-gradient-to-b from-navy-900 to-navy-950 flex flex-col border-s border-white/5">
      {/* Header */}
      <div className="h-16 flex items-center px-4 border-b border-white/10 flex-shrink-0">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-10 h-10 rounded-xl overflow-hidden flex-shrink-0 bg-white/5 flex items-center justify-center ring-1 ring-gold-500/40">
            {appInfo?.logo ? (
          ...
          <span className="font-bold text-white truncate text-sm leading-tight">
            {appInfo?.app_name || 'نظام المحاماة'}
          </span>
        </div>
        {/* Mobile close button */}
        {onMobileClose && (
          <button onClick={onMobileClose} className="lg:hidden mr-auto p-1.5 rounded-lg hover:bg-white/10 text-white/60 hover:text-white transition-colors">
      ...
      {/* Nav */}
      <nav className="scrollbar-navy flex-1 overflow-y-auto py-3 px-2.5 space-y-0.5">
        {navGroups.map((group, gi) => {
          ...
          if (!group.label) {
            return (
              <div key={gi} className="pb-1.5 mb-1.5 border-b border-white/10 last:border-0 last:mb-0 last:pb-0">
                {visibleItems.map(item => (
                  ...
                  >
                    ...
                    <svg className={`w-[18px] h-[18px] flex-shrink-0 ${isActive(item.href) ? 'text-gold-500' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={isActive(item.href) ? 2.25 : 2} d={item.icon} />
                    </svg>
                    {item.label}
                  </Link>
                ))}
          ...
          return (
            <div key={gi}>
              <button
                onClick={() => toggle(gi)}
                className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-sm font-semibold transition-all duration-200 ${
                  hasActive ? 'text-white bg-white/[0.06]' : 'text-white/50 hover:bg-white/[0.04] hover:text-white/80'
                }`}
              >
                <svg className={`w-4 h-4 flex-shrink-0 ${hasActive ? 'text-gold-500' : 'text-white/40'}`}
                  fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.75} d={group.icon} />
                </svg>
                <span className="flex-1 text-right truncate text-[11px] font-bold tracking-[0.08em] uppercase">{group.label}</span>
                <svg
                  className={`w-3.5 h-3.5 flex-shrink-0 transition-transform duration-300 ${isOpen ? 'rotate-180' : ''} ${hasActive ? 'text-gold-500/80' : 'text-white/30'}`}
                  fill="none" stroke="currentColor" viewBox="0 0 24 24"
                >
              ...
              >
                <div className="ms-4 border-s border-white/10 ps-2.5 py-1 space-y-0.5">
                  {visibleItems.map(item => (
                    <Link
                      key={item.href}
                      href={item.href}
                    ...
                    >
                      {isActive(item.href) && (
                        <span className="absolute -start-[11px] inset-y-1.5 w-[2px] rounded-full bg-gold-500" />
                      )}
                      <svg className={`w-3.5 h-3.5 flex-shrink-0 ${isActive(item.href) ? 'text-gold-500' : 'text-white/40'}`}
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={isActive(item.href) ? 2.25 : 2} d={item.icon} />
                      </svg>
      ...
      <div className="flex-shrink-0 p-3 border-t border-white/10">
        <div className="flex items-center gap-2.5 mb-2.5 px-1">
          <div className="w-9 h-9 rounded-full overflow-hidden flex-shrink-0 bg-white/5 flex items-center justify-center ring-1 ring-gold-500/40">
            {user?.profile_image ? (
              <img src={user.profile_image} alt="" className="w-full h-full object-cover" />
            ) : (
              <span className="text-gold-500 font-bold text-sm">{user?.name?.charAt(0) || 'م'}</span>
            )}
          </div>
        ...
        <Link href="/profile" onClick={onMobileClose}
          className="flex items-center gap-2 px-3 py-2 text-xs text-white/60 hover:bg-white/[0.06] hover:text-white rounded-lg transition-colors mb-1">
        ...
        <button onClick={logout}
          className="w-full flex items-center gap-2 px-3 py-2 text-xs text-red-300 hover:bg-red-500/15 hover:text-red-200 rounded-lg transition-colors">
          <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <>
      {/* Desktop */}
      <div className="hidden lg:flex flex-col w-64 min-h-screen flex-shrink-0 shadow-[4px_0_24px_rgba(8,26,58,0.12)]">
        {sidebarContent}
      </div>
      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-40 flex">
          <div className="fixed inset-0 bg-navy-950/60 backdrop-blur-sm" onClick={onMobileClose} />
